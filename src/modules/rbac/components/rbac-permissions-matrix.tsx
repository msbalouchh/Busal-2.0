...
import { saveRbacPermissionsAction } from "@/modules/rbac/actions/rbac-actions";
import {
...
export function RbacPermissionsMatrix({ matrix, canEdit }: RbacPermissionsMatrixProps) {
  ...
  const handleSave = () => {
    ...
    startTransition(async () => {
      try {
        await saveRbacPermissionsAction(payload);
  ...
  return (
    <div className="space-y-4" aria-label="Role permissions matrix">
      {canEdit ? (
        ...
        <p className="text-muted-foreground text-sm">You have view-only access to permissions.</p>
      )}

      <div className="overflow-x-auto rounded-lg border">
        <Table aria-label="Permissions by role">
          <TableHeader>
            <TableRow>
              ...
              {roles.map((role) => (
                <TableHead key={role.id} scope="col" className="min-w-[100px] text-center">
                  <div className="flex flex-col items-center gap-1">
                    <span>{role.name}</span>
                    {role.isSystem ? (
                      <span className="text-muted-foreground text-xs font-normal">System</span>
                    ) : (
                      <span className="text-muted-foreground text-xs font-normal">Custom</span>
                    )}
                  </div>
                </TableHead>
              ))}
                ...
                {groupPermissions.map((permission) => (
                  <TableRow key={permission.id}>
                    ...
                    {roles.map((role) => {
                      ...
                      return (
                        <TableCell key={inputId} className="text-center">
                          <input
                            id={inputId}
                            type="checkbox"
                            checked={isChecked}
                            ...
                            className="focus-visible:ring-ring h-4 w-4 focus-visible:ring-2 focus-visible:outline-none"
                          />
                        </TableCell>
                      );
                    })}
