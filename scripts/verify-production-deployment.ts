...
async function main() {
  ...
  assert(read("src/app/robots.ts").includes("sitemap"), "robots.ts missing sitemap reference");
  assert(read("src/app/sitemap.ts").includes("resolvePublicAppUrl"), "sitemap.ts misconfigured");
  console.log("  PASS");
